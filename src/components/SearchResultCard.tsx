    ...
    return (
        <Link to={`/detail/${restaurant._id}`} className="grid  lg:grid-cols-[2fr_3fr] gap-5 group">
            <AspectRatio ratio={16 / 6}>
                <img src={restaurant.imageUrl} alt={restaurant.restaurantName} className="rounded-md w-full h-full object-cover" />
            </AspectRatio>
            ...
            <div>
                ...
                <div className="grid md:grid-cols-2 gap-2">
                    <div className="flex flex-wrap">
                        {(restaurant?.cuisines || []).map((cuisineItem, index) => (
                            <span key={index} className="flex">
                                <span>{cuisineItem}</span>
